...
class JordanProjectManagerServer {
  ...
  /**
   * Process message for Jordan and return enhanced prompt for LLM
   */
  private async processMessageForJordan(message: string, context: string): Promise<string> {
    ...
    const enhancedPrompt = this.createEnhancedPrompt(jordanContext, message);
...
`;
  ...
  private createEnhancedPrompt(jordanContext: string, userMessage: string): string {
...
${!memoryData.aisdlcTraining.completed ?
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      if (name === "process-message-for-jordan") {
        ...
        try {
          ...
          const enhancedPrompt = await this.processMessageForJordan(message, context);
