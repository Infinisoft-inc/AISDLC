...
describe('ProcessMessageFor', () => {
  ...
  beforeEach(() => {
    processMessageFor = new ProcessMessageFor('Sarah', 'AI Business Analyst');
  });
  ...
  test('should have correct schema', () => {
    ...
    expect(schema.description).toContain('Process a message for Sarah');
  ...
  test('should execute with general context', async () => {
    ...
    expect(result.content[0].text).toContain('You are Sarah, AI Business Analyst');
